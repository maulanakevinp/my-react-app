...
const ProductCard = ({ children }) => {
  return (
    <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow mx-2 flex flex-col justify-between mb-5">
      {children}
    </div>
  );
};
...
const Body = ({ children, title }) => {
  return (
    <div className="p-5 h-full">
      ...
      <p className="mb-3 font-normal text-gray-700">{children.length > 100 ? children.slice(0, 100) + "..." : children}</p>
    </div>
  );
};
...
const Footer = ({ price, id }) => {
  ...
  return (
    <div className="flex justify-between m-5">
      <h3 className="mb-0 text-xl font-bold tracking-tight text-gray-900">Rp. {Intl.NumberFormat("id-ID").format(price)}</h3>
      <button
